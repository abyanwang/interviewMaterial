...
hashmap里面有一个entry table 用来存放链表的头，通过hash之后来判定属于那个index下，同时链表的长度超过8之后会变成红黑树，方便查询。

hashmap扩容的时候先判定size和临界值的大小关系，临界值通过容量和factor相乘得到，如果size大于临界值，进行rehash，是先开一个两倍容量的table，通过新的hash给放回去。

除了拉链法之外，主要使用头插法，在确定好那个index下后使用头插法来插入key-value
